...
bool CDVDVideoCodecHybris::Open(CDVDStreamInfo &hints, CDVDCodecOptions &options) {
  ...
  CLog::Log(LOGERROR, "%s::%s - Created format %ux%u", CLASSNAME, __func__, hints.width, hints.height);
  m_format = media_format_create_video_format(m_mimeType.c_str(), hints.width, hints.height, 0, 0);
  if (m_format == NULL) {
